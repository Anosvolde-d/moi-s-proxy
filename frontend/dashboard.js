...
function initializeDashboard() {
    ...
    loadRecentRequests();
    loadModelCosts(); // New
    loadAnnouncements(); // Load announcements for admin management
...
/**
 * Reset the daily quota for the current user (delete today's logs)
 * 
...
async function resetUserQuota() {
    ...
    // Confirm with user
    if (!confirm('Are you sure you want to reset this user\'s daily quota to $0.00? This will delete their usage logs for today.')) {
        return;
    }
    
    try {
        const response = await apiFetch(`/api/admin/sub-key/${currentUserDetailsSubKeyId}/reset-quota`, {
            method: 'POST'
        });
        
        if (!response.ok) {
            const data = await response.json();
            throw new Error(data.detail || 'Failed to reset quota');
        }
        
        const result = await response.json();
        
        // Update the display to show $0.00 usage
        document.getElementById('userDetailsDollarUsage').textContent = '$0.00';
        
        showToast(`Quota reset to $0.00 (${result.logs_deleted} logs deleted)`, 'success');
        ...
        showUserDetailsModal(currentUserDetailsSubKeyId);
...
/**
 * Load all announcements for admin management
...
async function loadAnnouncements() {
    ...
    try {
        ...
        listEl.innerHTML = announcements.map(ann => renderAnnouncementCard(ann)).join('');
...
/**
 * Render an announcement card for the admin list
 */
function renderAnnouncementCard(announcement) {
    ...
    if (announcement.expires_at) {
        ...
        expiryHtml = `
            <div class="announcement-card-expiry ${isExpired ? 'expired' : ''}">
                ${isExpired ? '⚠️ Expired' : '⏰ Expires'}: ${expiryDate.toLocaleString()}
            </div>
        `;
    }
        ...
        if (announcement.media_type === 'audio') {
            mediaHtml = `
                <div class="announcement-card-media">
                    <span class="media-badge">🔊 Audio attached</span>
                    <audio controls preload="metadata" style="width: 100%; margin-top: 8px;">
                        <source src="${escapeHtml(announcement.media_url)}" type="audio/mpeg">
                    </audio>
                </div>
            `;
        } else {
            mediaHtml = `
                <div class="announcement-card-media">
                    <span class="media-badge">🖼️ Image attached</span>
                    <img src="${escapeHtml(announcement.media_url)}" alt="Media preview" 
                         style="max-width: 200px; max-height: 150px; margin-top: 8px; border-radius: 8px; cursor: pointer;"
                         onclick="window.open('${escapeHtml(announcement.media_url)}', '_blank')">
                </div>
            `;
        }
    }
        ...
        const resultsHtml = options.map(option => {
            ...
            return `
                <div class="poll-result-item">
                    <span class="poll-result-label">${escapeHtml(option)}</span>
                    <div class="poll-result-bar-container">
                        <div class="poll-result-bar" style="width: ${percentage}%"></div>
                    </div>
                    <span class="poll-result-count">${count} (${percentage.toFixed(0)}%)</span>
                </div>
            `;
        }).join('');
        
        pollHtml = `
            <div class="announcement-poll-results">
                <h4>📊 Poll Results (${totalVotes} votes)</h4>
                ${resultsHtml}
            </div>
        `;
    }
    
    const typeIcon = getAnnouncementTypeIcon(type);
    
    return `
        <div class="announcement-card ${type}" data-id="${announcement.id}">
            <div class="announcement-card-header">
                <div class="announcement-card-title">
                    ${typeIcon} ${title}
                </div>
                <div class="announcement-card-meta">
                    <span class="announcement-card-type ${type}">${type.toUpperCase()}</span>
                    <span class="announcement-card-date">${createdAt}</span>
                </div>
            </div>
            <div class="announcement-card-content">${content}</div>
            ${mediaHtml}
            ${expiryHtml}
            ${pollHtml}
            <div class="announcement-card-actions">
                <button class="action-btn" onclick="editAnnouncement(${announcement.id})">
                    ✏️ Edit
                </button>
                <button class="action-btn" onclick="viewAnnouncementReactions(${announcement.id})">
                    😀 Reactions
                </button>
                <button class="action-btn danger" onclick="deleteAnnouncement(${announcement.id})">
                    🗑️ Delete
                </button>
            </div>
        </div>
...
/**
 * Show the create announcement modal
...
function showCreateAnnouncementModal() {
    ...
    // Reset media fields
    document.getElementById('announcementMediaUrl').value = '';
    document.getElementById('announcementMediaType').value = '';
...
async function editAnnouncement(announcementId) {
    ...
    try {
        ...
        if (announcement.type === 'poll' && announcement.poll_options) {
            document.getElementById('pollOptionsGroup').style.display = 'block';
            announcement.poll_options.forEach(option => {
        ...
        // Handle media fields
        document.getElementById('announcementMediaUrl').value = announcement.media_url || '';
        document.getElementById('announcementMediaType').value = announcement.media_type || '';
        
        // Update modal title
        document.getElementById('announcementModalTitle').textContent = '✏️ Edit Announcement';
...
/**
 * Toggle poll options visibility based on type selection
 * **Validates: Requirements 3.5**
 */
function togglePollOptions() {
    const type = document.getElementById('announcementType').value;
    const pollOptionsGroup = document.getElementById('pollOptionsGroup');
    const pollOptionsContainer = document.getElementById('pollOptionsContainer');
...
function addPollOption(value = '') {
    ...
    row.innerHTML = `
        <input type="text" class="form-input poll-option-input" placeholder="Option ${index + 1}" value="${escapeHtml(value)}">
        <button type="button" class="btn-remove" onclick="removePollOption(this)">✕</button>
    `;
...
async function saveAnnouncement() {
    ...
    const type = document.getElementById('announcementType').value;
    const expiryValue = document.getElementById('announcementExpiry').value;
    const mediaUrl = document.getElementById('announcementMediaUrl').value.trim();
    ...
    // Get poll options if type is poll
    let pollOptions = null;
    if (type === 'poll') {
        const optionInputs = document.querySelectorAll('.poll-option-input');
        pollOptions = Array.from(optionInputs)
    ...
    try {
        ...
        if (currentEditAnnouncementId) {
            // Update existing
            response = await apiFetch(`/api/admin/announcements/${currentEditAnnouncementId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            // Create new
            response = await apiFetch('/api/admin/announcements', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(body)
            });
        ...
        if (response.ok) {
            showToast(currentEditAnnouncementId ? 'Announcement updated' : 'Announcement created', 'success');
            closeAnnouncementModal();
            loadAnnouncements();
        } else {
            const error = await response.json();
            showToast(error.detail || 'Failed to save announcement', 'error');
        }
...
async function deleteAnnouncement(announcementId) {
    ...
    try {
        const response = await apiFetch(`/api/admin/announcements/${announcementId}`, {
            method: 'DELETE'
        });
        ...
        if (response.ok) {
            ...
            const error = await response.json();
            showToast(error.detail || 'Failed to delete announcement', 'error');
        }
...
/**
 * View reactions for an announcement
 */
async function viewAnnouncementReactions(announcementId) {
    try {
        const response = await apiFetch(`/api/admin/announcements/${announcementId}/reactions`);
        
        if (response.ok) {
            const data = await response.json();
            
            // Build reactions summary
            let reactionsHtml = '<div class="reactions-detail">';
            
            if (data.reactions && Object.keys(data.reactions).length > 0) {
                reactionsHtml += '<h4>Reactions Summary</h4>';
                reactionsHtml += '<div class="reactions-grid">';
                    ...
                    reactionsHtml += `
                        <div class="reaction-item">
                            ...
                            <div class="reaction-users">
                                ${users.map(u => `<span class="user-tag">${u.username || u.discord_id}</span>`).join('')}
                            </div>
                        </div>
                    `;
                }
                ...
                reactionsHtml += '<p class="no-reactions">No reactions yet</p>';
            }
            ...
            const error = await response.json();
            showToast(error.detail || 'Failed to load reactions', 'error');
        }
...
window.togglePollOptions = togglePollOptions;
window.addPollOption = addPollOption;
window.removePollOption = removePollOption;
window.saveAnnouncement = saveAnnouncement;
window.deleteAnnouncement = deleteAnnouncement;
window.viewAnnouncementReactions = viewAnnouncementReactions;
