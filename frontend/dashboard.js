...
function editKey(keyId, currentRpm, currentRpd, keyData) {
    try {
        ...
        if (keyData) {
            ...
            if (editUseProxy) editUseProxy.checked = keyData.use_proxy === undefined || keyData.use_proxy === null ? true : Boolean(keyData.use_proxy);
            ...
            if (editUseProxy) editUseProxy.checked = true;
...
function renderSubKeys(keyId, subKeys) {
    ...
    const html = subKeys.map(subKey => {
        ...
        return `
            <div class="sub-key-item ${statusClass}">
