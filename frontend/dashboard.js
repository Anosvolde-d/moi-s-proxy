...
async function loadRecentRequests() {
    try {
        ...
        if (logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="loading">No recent requests</td></tr>';
        } else {
        ...
        const tbody = document.getElementById('requestLogBody');
        tbody.innerHTML = '<tr><td colspan="8" class="loading" style="color: var(--danger-color);">Failed to load</td></tr>';
    }
}
...
async function loadTopIps() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="loading-text text-center">Loading top IPs...</td></tr>';

    try {
        const password = sessionStorage.getItem(ADMIN_PASSWORD_KEY);
        const response = await fetch('/api/top-ips?limit=10&days=30', {
            headers: { 'Authorization': `Bearer ${password}` }
        });

        if (!response.ok) throw new Error('Failed to load top IPs');

        const data = await response.json();
        const topIps = data.top_ips || [];

        if (topIps.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="loading-text text-center">No IP data available</td></tr>';
            return;
        }

        tbody.innerHTML = topIps.map((ip, index) => {
            const topApps = (ip.top_apps || []).map(app => 
                `<span class="app-badge">${escapeHtml(app.app)} (${app.requests})</span>`
            ).join(' ');
            
            return `
                <tr>
                    <td class="rank-cell">#${index + 1}</td>
                    <td class="ip-cell">${escapeHtml(ip.client_ip)}</td>
                    <td class="token-cell">${formatNumber(ip.total_tokens || 0)}</td>
                    <td class="request-cell">${formatNumber(ip.request_count || 0)}</td>
                    <td class="apps-cell">${topApps || '<span class="text-muted">None</span>'}</td>
                </tr>
            `;
        }).join('');
