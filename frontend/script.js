...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if user is logged in with Discord to show nav tabs
    checkDiscordAuth();
});
// ==================== GLOBAL ANALYTICS ====================
...
/**
 * Display global analytics data with count-up animation
 */
function displayGlobalAnalytics(data) {
    // Helper to set value with count-up animation
    const setWithCountUp = (elementId, value, suffix = '') => {
        const elem = document.getElementById(elementId);
        if (!elem) return;
        
        const formattedValue = formatNumber(value) + suffix;
        
        // Trigger count-up animation if available (defined in index.html)
        if (window.triggerCountUp && value > 0) {
            window.triggerCountUp(elementId, value, formattedValue);
        } else {
    ...
    // Total Requests
    setWithCountUp('globalTotalRequests', data.total_requests || 0);
    ...
    if (successRate) {
        ...
        if (window.triggerCountUp && rate > 0) {
            window.triggerCountUp('globalSuccessRate', rate, rateText);
        } else {
            successRate.textContent = rateText;
        }
    }
    
    // Total Tokens
    setWithCountUp('globalTotalTokens', data.total_tokens || 0);
    
    // Input Tokens
    setWithCountUp('globalInputTokens', data.input_tokens || 0);
    
    // Output Tokens
    setWithCountUp('globalOutputTokens', data.output_tokens || 0);
    
    // Top Model (no count-up for text)
    ...
    if (activeKeys) {
        const value = data.active_keys || 0;
        if (window.triggerCountUp && value > 0) {
            window.triggerCountUp('globalActiveKeys', value, formatNumber(value));
        } else {
            activeKeys.textContent = formatNumber(value);
        }
    }
}
...
async function handleValidate() {
    ...
    try {
        ...
        if (keyInfo.valid) {
            ...
            // Dispatch event with the actual API key for chatbot
            window.dispatchEvent(new CustomEvent('apiKeyValidated', { 
...
function displayKeyInfo(info) {
    ...
    // Dispatch event for chatbot to receive key info
    window.dispatchEvent(new CustomEvent('keyValidated', { 
        detail: {
            name: info.name,
            prefix: info.prefix,
            ...
            rpm: info.max_rpm,
            rpd: info.max_rpd,
            hasCustomTarget: info.has_custom_target
        }
    }));
}
...
/**
 * Check if user is logged in with Discord and show nav tabs
 */
async function checkDiscordAuth() {
    try {
        ...
        if (response.ok) {
            ...
            // User is logged in, show nav tabs
            const navTabs = document.getElementById('discordNavTabs');
            if (navTabs) {
            ...
            if (userInfo && userData) {
                ...
                if (userAvatar && userData.avatar) {
                    userAvatar.src = `https://cdn.discordapp.com/avatars/${userData.discord_id}/${userData.avatar}.png`;
                } else if (userAvatar) {
                    // Default avatar
                ...
                // Set username
                if (userName) {
                    userName.textContent = userData.global_name || userData.username || 'User';
                }
            }
        } else {
        ...
        // Not logged in or error, keep login button visible
        console.log('Discord auth check: not logged in');
    }
}
...
async function fetchAndDisplayStats(apiKey, keyInfo) {
    try {
        ...
        // Update chatbot with stats info
        window.dispatchEvent(new CustomEvent('keyStatsLoaded', { 
            detail: {
...
async function fetchAndDisplayModels(apiKey) {
    ...
    try {
        ...
        // Dispatch event for chatbot to know available models
        const modelNames = currentModels.map(m => typeof m === 'string' ? m : (m.id || m.name || 'Unknown'));
        window.dispatchEvent(new CustomEvent('modelsLoaded', { 
            detail: {
                models: modelNames,
                count: modelsData.count || 0,
                source: modelsData.source || 'default'
            }
        }));
    } catch (error) {
...
function displayUsageLogs(logs) {
    ...
    const html = logs.map(log => {
        // Ensure UTC timestamp is properly parsed and converted to local time
        const requestTime = log.request_time;
        const timestamp = new Date(requestTime.includes('Z') ? requestTime : requestTime + 'Z').toLocaleString();
