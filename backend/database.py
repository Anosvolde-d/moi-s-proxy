...
class CursorWrapper:
    """
    Wrapper to provide a consistent cursor interface for both SQLite and Turso.
    
    Implements proper async patterns for cursor operations.
    ...
    **Validates: Requirements 6.2**
    ...
    def rowcount(self):
        ...
        if self._result_set:
            return getattr(self._result_set, 'rowcount', len(self._result_set.rows) if hasattr(self._result_set, 'rows') else 0)
        return 0
    ...
    def lastrowid(self):
        ...
        if self._result_set:
            return getattr(self._result_set, 'last_insert_rowid', getattr(self._result_set, 'lastrowid', None))
        return None
    ...
    async def fetchone(self):
        ...
        **Validates: Requirements 6.2**
        ...
        Returns:
            dict or None: The next row as a dictionary, or None if no more rows
        """
        if self._exhausted:
            return None
        ...
        if self._result_set and hasattr(self._result_set, 'rows') and self._result_set.rows:
            if self._index < len(self._result_set.rows):
                row = self._result_set.rows[self._index]
                self._index += 1
                columns = self._result_set.columns if hasattr(self._result_set, 'columns') else []
        ...
        **Validates: Requirements 6.2**
        ...
        if self._result_set and hasattr(self._result_set, 'rows') and self._result_set.rows:
            ...
            if columns:
                ...
                remaining_rows = self._result_set.rows[self._index:]
                self._index = len(self._result_set.rows)
                self._exhausted = True
                return [dict(zip(columns, row)) for row in remaining_rows]
...
class ConnectionWrapper:
    """
    Wrapper to provide a consistent connection interface for both SQLite and Turso.
    
    Implements proper async patterns for database operations with error handling.
    ...
    **Validates: Requirements 6.1, 6.2**
    ...
    async def execute(self, sql, params=None):
        """
        Execute a SQL statement with proper async handling.
        ...
        **Validates: Requirements 6.2**
        ...
        Returns:
            CursorWrapper: Wrapper around the cursor/result set
            
        Raises:
            Exception: Re-raises database errors after logging
        """
        if self._closed:
            raise RuntimeError("Cannot execute on closed connection")
        ...
        if self._conn:
            ...
            try:
                ...
                logger.debug("Database execute cancelled")
                raise
            except Exception as e:
                logger.error(f"SQLite execute error: {e}")
                raise
            ...
            try:
                ...
                logger.debug("Turso execute cancelled")
                raise
            except Exception as e:
        ...
        **Validates: Requirements 6.2**
        ...
        if self._conn:
            try:
                ...
                logger.debug("Database commit cancelled")
                raise
            except Exception as e:
                logger.error(f"Error committing transaction: {e}")
                raise
        # libsql-client handles commits implicitly or via transactions
...
class Database:
    """Database abstraction layer supporting both SQLite and Turso."""
    
    def __init__(self, db_path: str = None):
        self.turso_available = LIB_SQL_AVAILABLE and bool(config.TURSO_DATABASE_URL)
        self.db_path = db_path or config.DATABASE_PATH
        self._turso_client = None
        self._turso_connected = False
        self._turso_retry_count = 0
        self._turso_last_error = None
        ...
        # Note: init_db() is async and must be called from startup_event
        # We initialize SQLite synchronously here for backward compatibility
        # Skip sync init for memory databases - they need async init
        if not self._is_memory_db:
            self._init_sqlite_sync()
    
    def _init_sqlite_sync(self):
        """Initialize SQLite database synchronously (for backward compatibility)."""
        try:
            conn = sqlite3.connect(self.db_path)
            self._init_tables(conn)
            conn.close()
            logger.info("SQLite database initialized synchronously")
        except Exception as e:
            logger.error(f"Failed to initialize SQLite synchronously: {e}")
    
    async def init_turso_client(self):
        """Initialize Turso client with retry logic (must be called from async context)."""
        if not self.turso_available:
            logger.info("Turso not available - skipping client initialization")
            return False
            
        if self._turso_client and self._turso_connected:
            logger.debug("Turso client already initialized and connected")
            return True
        ...
        for attempt in range(MAX_RETRY_ATTEMPTS):
            try:
                logger.info(f"Initializing Turso client (attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS})...")
                ...
                self._turso_connected = True
                self._turso_retry_count = 0
                self._turso_last_error = None
                logger.info("Turso client initialized and connected successfully")
                ...
                self._turso_connected = False
                self._turso_last_error = str(e)
                self._turso_retry_count = attempt + 1
                logger.error(f"Failed to initialize Turso client (attempt {attempt + 1}): {e}")
        ...
        # All retries failed
        logger.error(f"Failed to initialize Turso client after {MAX_RETRY_ATTEMPTS} attempts. Disabling Turso sync.")
        self.turso_available = False
        return False
    
    async def _reconnect_turso(self):
        """Attempt to reconnect to Turso after a connection failure."""
        if not LIB_SQL_AVAILABLE or not config.TURSO_DATABASE_URL:
            return False
            
        self._turso_connected = False
        self.turso_available = True  # Re-enable to allow retry
        return await self.init_turso_client()
    
    def get_turso_status(self) -> Dict[str, Any]:
        """Get current Turso connection status for diagnostics."""
        return {
            "available": self.turso_available,
            "connected": self._turso_connected,
            "retry_count": self._turso_retry_count,
            "last_error": self._turso_last_error,
            "url_configured": bool(config.TURSO_DATABASE_URL),
            "lib_available": LIB_SQL_AVAILABLE
        }

    async def _init_turso_tables(self):
        """Ensure all required tables exist in Turso."""
        if not self._turso_client or not self._turso_connected:
            return
    ...
    async def get_db(self):
        ...
        Always uses local SQLite for speed. Properly implements the async context
        manager protocol with correct error handling and resource cleanup.
        
        For in-memory databases, uses a shared connection to ensure data persistence
        across multiple calls.
        
        **Validates: Requirements 6.1, 6.2**
        
        Yields:
            ConnectionWrapper: A wrapper providing consistent interface for database operations
            
        Raises:
            Exception: Re-raises any connection errors after logging
        """
        db = None
        should_close = True
        try:
            if self._is_memory_db:
                ...
                db = self._shared_conn
                should_close = False  # Don't close shared connection
            else:
                db = await aiosqlite.connect(self.db_path)
                db.row_factory = aiosqlite.Row
            wrapper = ConnectionWrapper(conn=db)
            yield wrapper
        except aiosqlite.Error as e:
            ...
            # But don't close shared connections for in-memory databases
            if db is not None and should_close:
                try:
                    await db.close()
    ...
    async def _process_sync_queue(self):
        """Background task to sync local changes to Turso with error handling."""
        while True:
            try:
                ...
                if items and self._turso_client and self._turso_connected:
                    failed_items = []
                    for sql, params in items:
                        try:
                            await self._turso_client.execute(sql, params or ())
                        except Exception as e:
                            logger.warning(f"Turso sync error for query: {e}")
                            failed_items.append((sql, params))
                            # Check if this is a connection error
                            if "connection" in str(e).lower() or "network" in str(e).lower():
                                self._turso_connected = False
                                break
                    
                    if failed_items and not self._turso_connected:
                        # Re-queue failed items for retry after reconnection
                        for item in failed_items:
                            try:
                                self._sync_queue.put_nowait(item)
                            except asyncio.QueueFull:
                                logger.warning("Sync queue full, dropping failed item")
                        
                        # Attempt reconnection
                        logger.info("Attempting Turso reconnection...")
                        await self._reconnect_turso()
                    
                    if items and self._turso_connected:
                        logger.debug(f"Synced {len(items) - len(failed_items)} items to Turso")
    ...
    def queue_turso_sync(self, sql: str, params=None):
        ...
        if self.turso_available and self._turso_connected:
            try:
                self._sync_queue.put_nowait((sql, params))
            except asyncio.QueueFull:
                logger.warning(f"Turso sync queue full (size: {self._sync_queue.qsize()}). Dropping sync operation.")
            except Exception as e:
                logger.error(f"Unexpected error queuing Turso sync: {e}")
    
    async def load_from_turso(self):
        """Load all data from Turso into local SQLite (called once on startup)."""
        if not self.turso_available or not self._turso_connected or self._loaded_from_turso:
            if not self.turso_available:
                logger.info("Turso not available - skipping data load")
            elif not self._turso_connected:
                logger.warning("Turso not connected - skipping data load")
            return
                ...
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    for row in result.rows:
                        try:
                            ...
                            logger.warning(f"Error inserting api_key row: {e}")
                    await db.commit()
                ...
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    for row in result.rows:
                        try:
                            ...
                            logger.debug(f"Error inserting usage_log row: {e}")
                    await db.commit()
                ...
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    for row in result.rows:
                        try:
                            ...
                            logger.debug(f"Error inserting model_cost row: {e}")
                    await db.commit()
    ...
    async def full_sync_to_turso(self):
        ...
        if not self._turso_connected:
            logger.info("Turso not connected, attempting reconnection before sync...")
            if not await self._reconnect_turso():
                logger.warning("Failed to reconnect to Turso, skipping sync")
                return False
                ...
                if rows:
                    ...
                    for row in rows:
                        ...
                        try:
                            ...
                            logger.warning(f"Error syncing api_key to Turso: {e}")
                ...
                if rows:
                    ...
                    for row in rows:
                        ...
                        try:
                            ...
                            logger.debug(f"Error syncing usage_log to Turso: {e}")
                ...
                if rows:
                    ...
                    for row in rows:
                        ...
                        try:
                            ...
                            logger.warning(f"Error syncing model_cost to Turso: {e}")
    ...
    async def close(self):
        """Close the database clients and cleanup resources."""
        ...
        if self._shared_conn is not None:
            try:
                ...
                logger.warning(f"Error closing shared connection: {e}")
            self._shared_conn = None
    ...
    async def init_db(self):
        ...
        try:
            ...
            logger.error(f"Failed to initialize database asynchronously: {e}")
            # Fall back to sync initialization if async fails
            try:
                self._init_sqlite_sync()
            except Exception as e2:
                logger.error(f"Fallback sync initialization also failed: {e2}")
    ...
    async def invalidate_key_cache_by_id(self, key_id: int) -> bool:
        """
        Invalidate the cache for a specific API key by its ID.
        
        This ensures configuration updates are applied immediately to new requests.
        ...
        **Validates: Requirements 8.3**
        ...
        Returns:
            bool: True if cache was invalidated, False if key not found in cache
        """
        # Find and remove the cached entry for this key_id
        keys_to_remove = []
        for key_hash, (info, expiry) in self._key_cache.items():
            if info.get("id") == key_id:
                keys_to_remove.append(key_hash)
        
        for key_hash in keys_to_remove:
            del self._key_cache[key_hash]
            logger.debug(f"Invalidated cache for key ID {key_id}")
    ...
    async def get_api_key_by_id(self, key_id: int) -> Optional[Dict[str, Any]]:
        """Get an API key by its ID (without the actual key hash)"""
        async with self.get_db() as db:
            cursor = await db.execute('''
                SELECT id, key_prefix, name, max_rpm, max_rpd, enabled, created_at, last_used_at, 
                       target_url, target_api_key, use_proxy, no_auth, model_mappings, expires_at, 
                       ip_whitelist, ip_blacklist, providers, provider_rotation_index, 
                       provider_rotation_frequency, disable_model_fetch, http_referer, 
                       max_total_tokens, total_tokens_used, max_context_tokens, custom_prefills, 
                       budget_limit, budget_used, budget_reset_date
                FROM api_keys
                WHERE id = ?
            ''', (key_id,))
    ...
    async def delete_api_key(self, key_id: int) -> bool:
        """Delete an API key (alias for delete_key)"""
    ...
    async def update_key_limits(self, key_id: int, max_rpm: int = None, max_rpd: int = None,
        ...
        async with self.get_db() as db:
            ...
            # Invalidate cache to ensure configuration updates are applied immediately
            # **Validates: Requirements 8.3**
            if cursor.rowcount > 0:
                await self.invalidate_key_cache_by_id(key_id)
    ...
    async def calculate_cost(self, model: str, input_tokens: int, output_tokens: int) -> float:
        ...
        Uses pattern matching to find the most specific cost configuration.
        Returns 0.0 if no matching pattern is found or if tokens are 0.
        
        **Validates: Requirements 3.5** - Cost calculation accuracy
        """
        ...
        async with self.get_db() as db:
            ...
            # First try exact match, then pattern match
            cursor = await db.execute('''
                SELECT input_cost_per_1m, output_cost_per_1m, model_pattern
                FROM model_costs
            ...
            if row:
                input_cost_per_1m = float(row['input_cost_per_1m'] or 0)
                output_cost_per_1m = float(row['output_cost_per_1m'] or 0)
                input_cost = (input_tokens / 1_000_000.0) * input_cost_per_1m
                output_cost = (output_tokens / 1_000_000.0) * output_cost_per_1m
                return round(input_cost + output_cost, 6)  # Round to 6 decimal places for precision
    ...
    async def get_analytics(self, days: int = 7) -> Dict[str, Any]:
        ...
        All NULL values are properly handled with COALESCE to ensure accurate counting.
        """
        async with self.get_db() as db:
            db.row_factory = aiosqlite.Row
            
            # Total requests and tokens - use COALESCE for all numeric fields to ensure accuracy
            cursor = await db.execute('''
                SELECT
                    COUNT(*) as total_requests,
            ...
            # Requests per model - ensure accurate counting with COALESCE
            cursor = await db.execute('''
                SELECT 
            ...
            # Requests per day - ensure accurate counting
            cursor = await db.execute('''
                SELECT 
    ...
    async def check_and_increment_rate_limit(self, api_key: str) -> Dict[str, Any]:
        """
        Check and increment rate limit for an API key.
        
        This is a convenience method that validates the API key and checks rate limits.
