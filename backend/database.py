...
class Database:
        ...
        async with self._pool.acquire() as conn:
            ...
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS sub_keys (
                    ...
                    last_used_at TIMESTAMP,
                    enabled BOOLEAN DEFAULT TRUE,
                    daily_dollar_limit REAL DEFAULT 20.0,
                    daily_dollar_usage REAL DEFAULT 0.0,
                    daily_usage_reset_at TIMESTAMP DEFAULT NULL,
            ...
            # Migration: Add dollar limit columns to sub_keys if they don't exist
            try:
                await conn.execute('ALTER TABLE sub_keys ADD COLUMN IF NOT EXISTS daily_dollar_limit REAL DEFAULT 20.0')
                await conn.execute('ALTER TABLE sub_keys ADD COLUMN IF NOT EXISTS daily_dollar_usage REAL DEFAULT 0.0')
                await conn.execute('ALTER TABLE sub_keys ADD COLUMN IF NOT EXISTS daily_usage_reset_at TIMESTAMP DEFAULT NULL')
            except:
                pass
            ...
            settings_count = await conn.fetchval('SELECT COUNT(*) FROM global_settings')
            if settings_count == 0:
                await conn.execute(
                    "INSERT INTO global_settings (key, value) VALUES ('default_daily_dollar_limit', '20.0')"
                )
            
            # Migration: Create placeholder discord_users for legacy sub-keys that don't have one
            # This ensures the User Details modal works for sub-keys created before Discord auth
            try:
                orphan_rows = await conn.fetch('''
                    SELECT DISTINCT sk.discord_id 
                    FROM sub_keys sk 
                    LEFT JOIN discord_users du ON sk.discord_id = du.id 
                    WHERE du.id IS NULL AND sk.discord_id IS NOT NULL AND sk.discord_id != ''
                ''')
                
                for row in orphan_rows:
                    discord_id = row['discord_id']
                    # Create a placeholder discord_users record
                    await conn.execute('''
                        INSERT INTO discord_users (id, username, avatar, global_name, created_at)
                        VALUES ($1, $2, NULL, $3, NOW())
                        ON CONFLICT (id) DO NOTHING
                    ''', discord_id, f'Legacy User ({discord_id[:8]}...)', 'Legacy User')
                
                if orphan_rows:
                    logger.info(f"[Migration] Created {len(orphan_rows)} placeholder discord_users for legacy sub-keys")
            except Exception as e:
                logger.warning(f"[Migration] Could not create placeholder discord_users: {e}")
    ...
    async def _init_sqlite_tables(self, db):
        ...
        # Migration: Add dollar limit columns to sub_keys if they don't exist
        try:
            await db.execute('ALTER TABLE sub_keys ADD COLUMN daily_dollar_limit REAL DEFAULT 20.0')
        except:
            pass  # Column already exists
        try:
            await db.execute('ALTER TABLE sub_keys ADD COLUMN daily_dollar_usage REAL DEFAULT 0.0')
        except:
            pass  # Column already exists
        try:
            await db.execute('ALTER TABLE sub_keys ADD COLUMN daily_usage_reset_at TIMESTAMP DEFAULT NULL')
        except:
            pass  # Column already exists
        ...
        # Seed default settings if empty
        cursor = await db.execute('SELECT COUNT(*) FROM global_settings')
        row = await cursor.fetchone()
        if row[0] == 0:
            await db.execute(
                "INSERT INTO global_settings (key, value) VALUES ('default_daily_dollar_limit', '20.0')"
        ...
        try:
            cursor = await db.execute('''
                ...
                WHERE du.id IS NULL AND sk.discord_id IS NOT NULL AND sk.discord_id != ''
            ''')
            orphan_discord_ids = await cursor.fetchall()
            
            for row in orphan_discord_ids:
                discord_id = row[0]
                # Create a placeholder discord_users record
                await db.execute('''
                    INSERT OR IGNORE INTO discord_users (id, username, avatar, global_name, created_at)
                    VALUES (?, ?, NULL, ?, datetime('now'))
                ''', (discord_id, f'Legacy User ({discord_id[:8]}...)', f'Legacy User'))
            
            if orphan_discord_ids:
                await db.commit()
                print(f"[Migration] Created {len(orphan_discord_ids)} placeholder discord_users for legacy sub-keys")
        except Exception as e:
            print(f"[Migration] Warning: Could not create placeholder discord_users: {e}")
            ...
            async with self._pool.acquire() as conn:
                row = await conn.fetchrow('''
                    INSERT INTO api_keys (key_hash, key_prefix, name, max_rpm, max_rpd,
                        target_url, target_api_key, no_auth, model_mappings,
    ...
    async def get_all_keys(self) -> List[Dict[str, Any]]:
        """Get all API keys with real-time RPM calculated from usage_logs."""
        ...
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                ...
                # Calculate real-time RPM and RPD for each key from usage_logs
                for key in keys:
                    # Get RPM (requests in the last minute)
                    rpm = await conn.fetchval('''
                        SELECT COUNT(*) FROM usage_logs 
                        WHERE api_key_id = $1 AND request_time >= $2
                    ''', key['id'], minute_ago)
                    key['current_rpm'] = rpm or 0
                    
                    # Get RPD (requests today)
                    rpd = await conn.fetchval('''
                        SELECT COUNT(*) FROM usage_logs 
                        WHERE api_key_id = $1 AND request_time >= $2
                    ''', key['id'], today_start)
                    key['current_rpd'] = rpd or 0
            ...
            try:
                ...
                # Format timestamps for SQLite comparison
                ...
                for key in keys:
                    ...
                    cursor = await db.execute('''
                        SELECT COUNT(*) FROM usage_logs 
                        WHERE api_key_id = ? AND request_time >= ?
                    ''', (key['id'], minute_ago_str))
                    row = await cursor.fetchone()
                    key['current_rpm'] = row[0] if row else 0
                    
                    # Get RPD (requests today)
                    cursor = await db.execute('''
                        SELECT COUNT(*) FROM usage_logs 
                        WHERE api_key_id = ? AND request_time >= ?
                    ''', (key['id'], today_start_str))
                    row = await cursor.fetchone()
                    key['current_rpd'] = row[0] if row else 0
    ...
    async def get_sub_key_details_with_logs(self, sub_key_id: int, limit: int = 20) -> Optional[Dict[str, Any]]:
        """
        Get detailed sub-key information including user info, usage stats, and recent logs.
        
        This function returns comprehensive details for the user details modal:
        ...
        Returns:
            Dict with user info, usage stats, and logs, or None if not found
            
        **Validates: Requirements 1.2, 1.3, 1.4, 3.5**
        """
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                # Get sub-key info with Discord user details (LEFT JOIN to handle missing discord users)
                sub_key_row = await conn.fetchrow('''
                    SELECT 
                        sk.id as sub_key_id,
                        sk.sub_key_prefix,
                        ...
                        sk.last_used_at,
                        sk.enabled,
                        sk.daily_dollar_limit,
                        sk.daily_dollar_usage,
                        sk.daily_usage_reset_at,
                        sk.master_key_id,
                        COALESCE(du.username, 'Unknown') as discord_username,
                ...
                sub_key_info = dict(sub_key_row)
                
                # Get total request count for this sub-key
                total_requests = await conn.fetchval('''
                    SELECT COUNT(*) FROM usage_logs WHERE sub_key_id = $1
                ''', sub_key_id)
                
                # Get recent logs for this sub-key
                log_rows = await conn.fetch('''
                    SELECT 
                        ul.id,
                        ul.request_time as timestamp,
                        ul.model,
                        ul.input_tokens,
                        ul.output_tokens,
                        (ul.input_tokens + ul.output_tokens) as total_tokens,
                        ul.client_app,
                        ul.cost
                    FROM usage_logs ul
                    WHERE ul.sub_key_id = $1
            ...
            try:
                ...
                # Get total request count for this sub-key
                cursor = await db.execute('''
                    SELECT COUNT(*) FROM usage_logs WHERE sub_key_id = ?
                ''', (sub_key_id,))
                count_row = await cursor.fetchone()
                total_requests = count_row[0] if count_row else 0
    ...
    async def check_and_update_dollar_usage(self, sub_key_id: int, cost: float) -> Dict[str, Any]:
        """
        Check if a sub-key has exceeded its daily dollar limit and update usage.
        
        This function:
        1. Checks if the daily usage needs to be reset (past midnight UTC)
        2. If usage + cost > limit, rejects the request
        3. Otherwise, updates usage and allows the request
        
        Args:
            sub_key_id: The sub-key ID
            cost: The cost of the current request in dollars
            
        Returns:
            Dict with:
                - allowed: bool - whether the request is allowed
                - current_usage: float - current daily usage
                - limit: float - daily dollar limit
                - reset_at: str - when the limit resets (ISO format)
        ...
        **Validates: Requirements 3.2, 3.4**
        ...
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                # Get current sub-key info
                row = await conn.fetchrow('''
                    SELECT daily_dollar_limit, daily_dollar_usage, daily_usage_reset_at
                    FROM sub_keys WHERE id = $1
                ''', sub_key_id)
                
                if not row:
                    return {
                        "allowed": False,
                        "error": "Sub-key not found",
                        "current_usage": 0,
                        "limit": 0,
                        "reset_at": next_midnight.isoformat()
                    }
                
                limit = row['daily_dollar_limit'] or 20.0
                current_usage = row['daily_dollar_usage'] or 0.0
                reset_at = row['daily_usage_reset_at']
                ...
                if reset_at is None:
                    ...
                    if isinstance(reset_at, str):
                        try:
                            ...
                            reset_dt = datetime.now(timezone.utc) - timedelta(days=1)
                    else:
                        reset_dt = reset_at.replace(tzinfo=timezone.utc) if reset_at.tzinfo is None else reset_at
                ...
                if needs_reset:
                    ...
                    await conn.execute('''
                        UPDATE sub_keys 
                        SET daily_dollar_usage = 0.0, daily_usage_reset_at = $1
                        WHERE id = $2
                    ''', next_midnight, sub_key_id)
                
                # Check if adding this cost would exceed the limit
                if current_usage + cost > limit:
                    return {
                        "allowed": False,
                        "error": f"Daily usage limit of ${limit:.2f} exceeded. Current usage: ${current_usage:.2f}",
                ...
                await conn.execute('''
                    UPDATE sub_keys 
                    SET daily_dollar_usage = $1
                    WHERE id = $2
                ''', new_usage, sub_key_id)
            ...
            try:
                ...
                if needs_reset:
                    ...
                    await db.execute('''
                        UPDATE sub_keys 
                        SET daily_dollar_usage = 0.0, daily_usage_reset_at = ?
                        WHERE id = ?
                    ''', (next_midnight.isoformat(), sub_key_id))
                    await db.commit()
    ...
    async def get_sub_key_dollar_usage(self, sub_key_id: int) -> Dict[str, Any]:
        """
        Get the current dollar usage for a sub-key by summing today's logs.
        
        This is the simple, reliable approach: just sum the cost column from
        usage_logs for this sub_key_id where timestamp is today (UTC).
        
        Args:
            sub_key_id: The sub-key ID
            
        Returns:
            Dict with current_usage, limit, and reset_at
        ...
        **Validates: Requirements 3.5**
        ...
        if self._use_postgres:
            try:
                async with self._pool.acquire() as conn:
                    # Get the limit from sub_keys table
                    limit_row = await conn.fetchrow('''
                        SELECT daily_dollar_limit FROM sub_keys WHERE id = $1
                    ''', sub_key_id)
                    
                    if not limit_row:
                        return {"current_usage": 0, "limit": 20.0, "reset_at": next_midnight.isoformat()}
                    
                    limit = limit_row['daily_dollar_limit'] or 20.0
                    
                    # Sum today's costs from usage_logs
                    ...
                    current_usage = float(usage_row['total_cost']) if usage_row else 0.0
                    
                    logger.debug(f"[DollarUsage] sub_key_id={sub_key_id}, calculated from logs: ${current_usage:.4f}")
                ...
                logger.error(f"Error in get_sub_key_dollar_usage (postgres): {e}")
                return {"current_usage": 0, "limit": 20.0, "reset_at": next_midnight.isoformat()}
        else:
            db = self._shared_conn if self._is_memory_db else await aiosqlite.connect(self.db_path)
            try:
                ...
                # Get the limit from sub_keys table
                cursor = await db.execute('''
                    SELECT daily_dollar_limit FROM sub_keys WHERE id = ?
                ''', (sub_key_id,))
                ...
                cursor = await db.execute('''
                    ...
                    AND request_time >= ?
                    AND success = 1
                ''', (sub_key_id, today_midnight.isoformat()))
                ...
                logger.error(f"Error in get_sub_key_dollar_usage (sqlite): {e}")
                return {"current_usage": 0, "limit": 20.0, "reset_at": next_midnight.isoformat()}
            finally:
                if not self._is_memory_db:
                    await db.close()

    async def update_sub_key_dollar_limit(self, sub_key_id: int, new_limit: float) -> bool:
        """
        Update the daily dollar limit for a sub-key.
        
        Args:
            sub_key_id: The sub-key ID
            new_limit: The new daily dollar limit
        ...
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                result = await conn.execute('''
                    UPDATE sub_keys SET daily_dollar_limit = $1 WHERE id = $2
                ''', new_limit, sub_key_id)
                return 'UPDATE' in result
        else:
            db = self._shared_conn if self._is_memory_db else await aiosqlite.connect(self.db_path)
            try:
                await db.execute('''
                    UPDATE sub_keys SET daily_dollar_limit = ? WHERE id = ?
                ''', (new_limit, sub_key_id))
                await db.commit()
                return True
            finally:
    ...
    async def log_usage(self, key_id: int, model: str, input_tokens: int, output_tokens: int,
        ...
        Also updates sub-key daily dollar usage when a successful request is logged.
        ...
        **Validates: Requirements 5.1, 3.2**
        ...
        logger.debug(f"[log_usage] key_id={key_id}, model={model}, cost=${cost:.4f}, sub_key_id={sub_key_id}, success={success}")
        ...
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                ...
                # Update sub-key daily dollar usage if this is a sub-key request
                # **Validates: Requirements 3.2**
                if sub_key_id and success and cost > 0:
                    logger.info(f"[log_usage] Updating sub_key_id={sub_key_id} daily_dollar_usage += ${cost:.4f}")
                    
                    # Use a transaction to ensure the update is committed
                    async with conn.transaction():
                        await conn.execute('''
                            UPDATE sub_keys 
                            SET daily_dollar_usage = COALESCE(daily_dollar_usage, 0) + $1
                            WHERE id = $2
                        ''', cost, sub_key_id)
                    
                    # Verify the update
                    verify_row = await conn.fetchrow('''
                        SELECT daily_dollar_usage, daily_usage_reset_at
                        FROM sub_keys WHERE id = $1
                    ''', sub_key_id)
                    if verify_row:
                        logger.info(f"[log_usage] After update: sub_key_id={sub_key_id}, daily_dollar_usage=${verify_row['daily_dollar_usage']:.4f}, reset_at={verify_row['daily_usage_reset_at']}")
        else:
            db = self._shared_conn if self._is_memory_db else await aiosqlite.connect(self.db_path)
            try:
                await db.execute('''
                ...
                if sub_key_id and success and cost > 0:
                    await db.execute('''
                        UPDATE sub_keys 
                        SET daily_dollar_usage = COALESCE(daily_dollar_usage, 0) + ?
                        WHERE id = ?
                    ''', (cost, sub_key_id))
    ...
    async def _calculate_cost(self, model: str, input_tokens: int, output_tokens: int) -> float:
        ...
        # Default cost if no pattern matches - use a reasonable default rate
        # This ensures usage is always tracked even for unknown models
        # Default: $3/1M input, $15/1M output (similar to Claude Sonnet pricing)
        default_input_cost = (input_tokens / 1_000_000) * 3.0
        default_output_cost = (output_tokens / 1_000_000) * 15.0
        return default_input_cost + default_output_cost

    async def calculate_request_cost(self, input_tokens: int, output_tokens: int, model: str) -> float:
        """
        Calculate the cost of a request based on token counts and model.
        
        This is a public wrapper around _calculate_cost for use in dollar limit enforcement.
        Uses the model_costs table to look up pricing per model.
        ...
        **Validates: Requirements 3.2**
        """
        return await self._calculate_cost(model, input_tokens, output_tokens)
    ...
    async def get_key_request_logs(self, key_id: int = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        Includes Discord username for user attribution.
            ...
            async with self._pool.acquire() as conn:
                if key_id:
                    rows = await conn.fetch('''
                        SELECT ul.*, du.username as discord_username, du.global_name as discord_global_name
                        FROM usage_logs ul
                        LEFT JOIN discord_users du ON ul.discord_id = du.id
    ...
    async def get_key_usage(self, key_id: int) -> Optional[Dict[str, Any]]:
        """Get usage statistics for a specific key with real-time RPM/RPD from usage_logs."""
        ...
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                ...
                # Calculate real-time RPM (requests in the last minute)
                rpm = await conn.fetchval('''
                    SELECT COUNT(*) FROM usage_logs 
                    WHERE api_key_id = $1 AND request_time >= $2
                ''', key_id, minute_ago)
                
                # Calculate real-time RPD (requests today)
                rpd = await conn.fetchval('''
                    SELECT COUNT(*) FROM usage_logs 
                    WHERE api_key_id = $1 AND request_time >= $2
                ''', key_id, today_start)
            ...
            try:
                ...
                cursor = await db.execute('''
                    SELECT COUNT(*) FROM usage_logs 
                    WHERE api_key_id = ? AND request_time >= ?
                ''', (key_id, today_start_str))
                row = await cursor.fetchone()
                rpd = row[0] if row else 0
    ...
    async def set_global_setting(self, key: str, value: str) -> bool:
    ...
    async def get_all_global_settings(self) -> Dict[str, str]:
        ...
        if self._use_postgres:
            ...
            db = self._shared_conn if self._is_memory_db else await aiosqlite.connect(self.db_path)
            try:
                cursor = await db.execute('SELECT key, value FROM global_settings')
                rows = await cursor.fetchall()
    ...
    async def update_all_sub_keys_dollar_limit(self, new_limit: float) -> int:
        """Update the daily dollar limit for ALL sub-keys. Returns count of updated rows."""
        if self._use_postgres:
            async with self._pool.acquire() as conn:
                result = await conn.execute(
                    'UPDATE sub_keys SET daily_dollar_limit = $1', new_limit
                )
                # PostgreSQL returns "UPDATE N" where N is the count
                return int(result.split()[-1]) if result else 0
        else:
            db = self._shared_conn if self._is_memory_db else await aiosqlite.connect(self.db_path)
            try:
                cursor = await db.execute(
                    'UPDATE sub_keys SET daily_dollar_limit = ?', (new_limit,)
                )
                await db.commit()
