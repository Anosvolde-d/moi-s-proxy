...
# Known client patterns for User-Agent matching
# **Validates: Requirements 2.1, 2.2, 2.3**
CLIENT_PATTERNS = {
    "SillyTavern": [
        r"SillyTavern",
        r"ST-Client",
        r"node-fetch.*SillyTavern",
        r"sillytavern",
        r"silly-tavern",
        r"silly tavern",
    ],
    ...
    "ST/Tavo": [
        r"^node-fetch",  # node-fetch is typically SillyTavern or Tavo
        r"^node$",
    ],
}


def detect_client_app(request: Request) -> str:
    """
    Detect the client application from request headers.
    
    Priority order:
    1. X-Client-App custom header (explicit client identification)
    2. SillyTavern-specific headers (X-ST-Version, etc.)
    3. User-Agent header patterns for known clients
    ...
    Returns:
        Detected client application name, or "Unknown" if not identified
        
    **Validates: Requirements 2.1, 2.2, 2.3, 2.4**
    """
    # 1. Check X-Client-App header first (explicit client identification)
    x_client_app = request.headers.get("X-Client-App", "").strip()
    if x_client_app:
        # Validate against known clients for consistency
        x_client_lower = x_client_app.lower()
        for client_name, patterns in CLIENT_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, x_client_lower, re.IGNORECASE):
                    return client_name
        # If X-Client-App is set but doesn't match known patterns,
        # return it as-is (trust the explicit header)
        return x_client_app
    
    # 2. Check for SillyTavern-specific headers
    # SillyTavern sends custom headers like X-ST-Version or has specific patterns
    if request.headers.get("X-ST-Version") or request.headers.get("x-st-version"):
        return "SillyTavern"
    
    # Check for common SillyTavern request patterns
    # SillyTavern often sends requests with specific content patterns
    referer = request.headers.get("Referer", "").lower()
    origin = request.headers.get("Origin", "").lower()
    
    # Check if request comes from localhost (common for SillyTavern)
    if "localhost" in referer or "127.0.0.1" in referer:
        # If it's from localhost with node-fetch, likely SillyTavern
        user_agent = request.headers.get("User-Agent", "").strip().lower()
        if "node-fetch" in user_agent or "node" in user_agent:
            return "SillyTavern"
    
    # 3. Check User-Agent header for known client patterns
    ...
    # 4. Fallback to "Unknown" instead of exposing internal details
...
async def safe_async_http_request(
    ...
    **Validates: Requirements 6.3**
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="the proxy is having a seizure.... if i didnt already get informed, ping me in dc (help thread)")
...
async def forward_streaming_request(client_request: Dict[str, Any], api_key_id: int, target_url: str = None, target_api_key: str = None, no_auth: bool = False, client_ip: str = "unknown", http_referer: str = None, client_app: str = None, discord_id: str = None, sub_key_id: int = None):
    ...
    # Always send requests directly to target API
    final_url = url
...
async def forward_non_streaming_request(client_request: Dict[str, Any], api_key_id: int, target_url: str = None, target_api_key: str = None, no_auth: bool = False, client_ip: str = "unknown", http_referer: str = None, client_app: str = None, discord_id: str = None, sub_key_id: int = None):
...
async def chat_completions(request: Request):
    ...
    try:
        ...
        # Detect client application using the enhanced detection function
        # This checks X-Client-App header first, then User-Agent patterns
        # **Validates: Requirements 2.1, 2.2, 2.3, 2.4**
        ...
        if await db.is_client_blacklisted(client_app):
            ...
            raise HTTPException(status_code=403, detail=f"Huh, what's that bizarre frontend '{client_app}'? If it isn't anything weird, just ping me in my thread")
        ...
        if not api_key:
            ...
            raise HTTPException(status_code=401, detail="look at your api key's brotha, it's probably empty...")
        ...
        if not key_info:
            ...
            raise HTTPException(status_code=401, detail="That's the wrong key you dumbass! (love you btw.)")
        ...
        if is_sub_key and discord_id:
            ...
            if is_banned:
                ...
                raise HTTPException(status_code=403, detail="You got banned bitch, hehe")
        ...
        if not key_info["enabled"]:
            ...
            raise HTTPException(status_code=403, detail="key disabled or in maintenance srry")
        ...
        if not is_variation and key_info["max_total_tokens"] is not None:
            if key_info["total_tokens_used"] >= key_info["max_total_tokens"]:
                ...
                raise HTTPException(status_code=403, detail="calm down on the tokens my guy, the daily tokens for the key is out")
        
        # Check Daily Dollar Limit for sub-keys
        # **Validates: Requirements 3.2, 3.4**
        if is_sub_key and sub_key_id:
            dollar_usage = await db.get_sub_key_dollar_usage(sub_key_id)
            if dollar_usage["current_usage"] >= dollar_usage["limit"]:
                await broadcast_log(
                    f"Daily dollar limit exceeded for sub-key {sub_key_id}: ${dollar_usage['current_usage']:.2f} / ${dollar_usage['limit']:.2f}",
                    "WARNING"
                )
                raise HTTPException(
                    status_code=429,
                    detail={
                        ...
                        "message": f"Daily usage limit of ${dollar_usage['limit']:.2f} exceeded. Current usage: ${dollar_usage['current_usage']:.2f} (you can request for more, but don't forget that i can just decline 😑)",
                        "reset_at": dollar_usage["reset_at"]
                    }
                )
        ...
        # Get custom target URL, API key, provider name, no_auth flag, and http_referer from key_info
        ...
        await broadcast_log(
            f"{target_log_msg} | {target_key_log} | {no_auth_log} | Model: {body.get('model', 'unknown')} | Stream: {body.get('stream', False)} | IP: {client_ip} | Client: {client_app}"
        )
        
        # Forward request based on streaming preference
        if is_streaming:
            return await forward_streaming_request(body, key_info["id"], custom_target_url, custom_target_api_key, no_auth, client_ip, http_referer, client_app, discord_id, sub_key_id)
        else:
            response_data = await forward_non_streaming_request(body, key_info["id"], custom_target_url, custom_target_api_key, no_auth, client_ip, http_referer, client_app, discord_id, sub_key_id)
        ...
        raise HTTPException(status_code=500, detail=f"the proxy is having a seizure.... if i didnt already get informed, ping me in dc (help thread) - Error: {str(e)}")
...
async def generate_api_key(key_data: APIKeyCreate):
    ...
    try:
        ...
        await broadcast_log(f"New API key generated: {new_key['prefix']} - Target: {key_data.target_url or 'default'}{no_auth_str}{model_fetch_str}{providers_log_str}{claim_code_str}")
        return new_key
    except Exception as e:
    ...
    **Validates: Requirements 9.3, 9.4**
    ...
    try:
        ...
        for sub_key in sub_keys:
            ...
            # Get dollar usage info for this sub-key
            dollar_usage = await db.get_sub_key_dollar_usage(sub_key.get('sub_key_id'))
            ...
            formatted_sub_keys.append({
                ...
                "daily_dollar_usage": dollar_usage.get('current_usage', 0),
                "daily_dollar_limit": dollar_usage.get('limit', 20.0),
                "daily_usage_reset_at": dollar_usage.get('reset_at')
            })
...
class SubKeyLimitUpdate(BaseModel):
    """Request model for updating sub-key dollar limit."""
    daily_dollar_limit: float


@app.put("/api/admin/sub-key/{sub_key_id}/limit", dependencies=[Depends(verify_admin)])
async def update_sub_key_limit(sub_key_id: int, update: SubKeyLimitUpdate):
    """
    Update the daily dollar limit for a sub-key.
    
    Args:
        sub_key_id: The sub-key ID
        update: Object containing the new daily_dollar_limit
        
    Returns:
        Updated sub-key info with new limit
    ...
    **Validates: Requirements 3.3**
    ...
    try:
        ...
        # Validate the limit
        if update.daily_dollar_limit < 0:
            raise HTTPException(status_code=400, detail="Daily dollar limit must be non-negative")
        
        # Update the limit
        success = await db.update_sub_key_dollar_limit(sub_key_id, update.daily_dollar_limit)
        if not success:
            raise HTTPException(status_code=500, detail="Failed to update dollar limit")
        
        # Get updated usage info
        dollar_usage = await db.get_sub_key_dollar_usage(sub_key_id)
        
        await broadcast_log(
            f"Updated dollar limit for sub-key {sub_key_id} to ${update.daily_dollar_limit:.2f}",
            "INFO"
        )
        
        return {
            "sub_key_id": sub_key_id,
            "daily_dollar_limit": update.daily_dollar_limit,
            "daily_dollar_usage": dollar_usage.get('current_usage', 0),
            "daily_usage_reset_at": dollar_usage.get('reset_at'),
            "message": f"Dollar limit updated to ${update.daily_dollar_limit:.2f}"
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error updating sub-key limit: {e}")
        await broadcast_log(f"Error updating sub-key limit: {str(e)}", "ERROR")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/admin/sub-key/{sub_key_id}/usage", dependencies=[Depends(verify_admin)])
async def get_sub_key_usage(sub_key_id: int):
    """
    Get the current dollar usage for a sub-key.
    
    Args:
        sub_key_id: The sub-key ID
        
    Returns:
        Current usage, limit, and reset time
    ...
    **Validates: Requirements 3.5**
    ...
    try:
        ...
        return {
            ...
            "daily_dollar_limit": dollar_usage.get('limit', 20.0),
            "daily_usage_reset_at": dollar_usage.get('reset_at'),
            "usage_percentage": (dollar_usage.get('current_usage', 0) / dollar_usage.get('limit', 20.0)) * 100 if dollar_usage.get('limit', 20.0) > 0 else 0
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting sub-key usage: {e}")
        await broadcast_log(f"Error getting sub-key usage: {str(e)}", "ERROR")
        raise HTTPException(status_code=500, detail=str(e))
...
class GlobalSubKeySettings(BaseModel):
    """Request model for updating global sub-key settings."""
    default_daily_dollar_limit: float


@app.get("/api/admin/settings/sub-keys", dependencies=[Depends(verify_admin)])
async def get_global_sub_key_settings():
    """
    Get global settings for sub-keys.
    
    Returns:
        Current default daily dollar limit for new sub-keys
    """
    return {
        "default_daily_dollar_limit": config.DEFAULT_DAILY_DOLLAR_LIMIT
    }


@app.put("/api/admin/settings/sub-keys", dependencies=[Depends(verify_admin)])
async def update_global_sub_key_settings(settings: GlobalSubKeySettings):
    """
    Update global settings for sub-keys and optionally apply to all existing sub-keys.
    
    Args:
        settings: Object containing the new default_daily_dollar_limit
        
    Returns:
        Updated settings and count of affected sub-keys
    """
    try:
        if settings.default_daily_dollar_limit < 0:
            raise HTTPException(status_code=400, detail="Daily dollar limit must be non-negative")
        
        # Update the config (runtime only - for persistence, update .env)
        config.DEFAULT_DAILY_DOLLAR_LIMIT = settings.default_daily_dollar_limit
        
        # Update all existing sub-keys with the new limit
        updated_count = await db.update_all_sub_keys_dollar_limit(settings.default_daily_dollar_limit)
        
        await broadcast_log(
            f"Updated global daily dollar limit to ${settings.default_daily_dollar_limit:.2f} - {updated_count} sub-keys updated",
            "INFO"
        )
        
        return {
            "default_daily_dollar_limit": settings.default_daily_dollar_limit,
            "sub_keys_updated": updated_count,
            "message": f"Global limit updated to ${settings.default_daily_dollar_limit:.2f}, {updated_count} sub-keys updated"
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error updating global sub-key settings: {e}")
        await broadcast_log(f"Error updating global sub-key settings: {str(e)}", "ERROR")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/admin/settings/global", dependencies=[Depends(verify_admin)])
async def get_global_settings():
    """
    Get all global settings from the database.
    
    Returns:
        Dictionary of all global settings including default_daily_dollar_limit
    """
    try:
        ...
        return {
            "default_daily_dollar_limit": float(settings.get('default_daily_dollar_limit', '20.0'))
        }
    except Exception as e:
...
@app.put("/api/admin/settings/global", dependencies=[Depends(verify_admin)])
async def update_global_settings(settings: dict):
    ...
    """
    try:
        if 'default_daily_dollar_limit' in settings:
            new_limit = float(settings['default_daily_dollar_limit'])
            if new_limit < 0:
                raise HTTPException(status_code=400, detail="Daily dollar limit must be non-negative")
            
            await db.set_global_setting('default_daily_dollar_limit', str(new_limit))
            
            # Also update the runtime config
            config.DEFAULT_DAILY_DOLLAR_LIMIT = new_limit
            
            await broadcast_log(f"Updated default daily dollar limit to ${new_limit:.2f}", "INFO")
...
@app.put("/api/admin/sub-keys/bulk-limit", dependencies=[Depends(verify_admin)])
async def update_all_sub_keys_limit(data: dict):
    """
    Update the daily dollar limit for ALL existing sub-keys at once.
    
    Args:
        data: Dictionary containing daily_dollar_limit
        
    Returns:
        Count of updated sub-keys
    """
    try:
        new_limit = float(data.get('daily_dollar_limit', 20.0))
        if new_limit < 0:
            raise HTTPException(status_code=400, detail="Daily dollar limit must be non-negative")
        
        updated_count = await db.update_all_sub_keys_dollar_limit(new_limit)
        
        await broadcast_log(f"Bulk updated {updated_count} sub-keys to ${new_limit:.2f} daily limit", "INFO")
        
        return {
            "updated_count": updated_count,
            "daily_dollar_limit": new_limit,
            "message": f"Updated {updated_count} sub-keys to ${new_limit:.2f} daily limit"
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error bulk updating sub-key limits: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/admin/sub-key/{sub_key_id}/details", dependencies=[Depends(verify_admin)])
async def get_sub_key_details(sub_key_id: int, limit: int = 20):
    """
    Get detailed sub-key information including user info, usage stats, and recent logs.
    
    This endpoint returns comprehensive details for the user details modal:
    - User info (Discord avatar, username, sub-key prefix)
    - Usage stats (daily dollar usage, limit, total requests)
    - Recent usage logs with tokens, model, client app, and cost
    
    Args:
        sub_key_id: The sub-key ID
        limit: Maximum number of logs to return (default 20, max 50)
        
    Returns:
        JSON with user info, usage stats, and logs
        
    **Validates: Requirements 1.2, 1.3, 1.4, 3.5**
    """
    try:
        # Validate limit
        limit = min(max(1, limit), 50)  # Clamp between 1 and 50
        
        # Get detailed sub-key info with logs
        details = await db.get_sub_key_details_with_logs(sub_key_id, limit)
        
        if not details:
            raise HTTPException(status_code=404, detail="Sub-key not found")
        
        sub_key_info = details["sub_key_info"]
        
        # Get the CORRECT dollar usage (with reset logic applied)
        # This ensures the reset is applied if needed before displaying
        dollar_usage = await db.get_sub_key_dollar_usage(sub_key_id)
        
        # Build avatar URL
        discord_id = sub_key_info.get('discord_id', '')
        avatar = sub_key_info.get('discord_avatar')
        if avatar:
            avatar_url = f"https://cdn.discordapp.com/avatars/{discord_id}/{avatar}.png"
        else:
            avatar_url = f"https://cdn.discordapp.com/embed/avatars/{int(discord_id) % 5 if discord_id.isdigit() else 0}.png"
        ...
        # Use global_name if available, otherwise username
        display_name = sub_key_info.get('discord_global_name') or sub_key_info.get('discord_username', 'Unknown')
        ...
        for log in details["logs"]:
            formatted_logs.append({
                "id": log.get('id'),
                "timestamp": str(log.get('timestamp')) if log.get('timestamp') else None,
                "model": log.get('model', 'Unknown'),
                "input_tokens": log.get('input_tokens', 0),
                "output_tokens": log.get('output_tokens', 0),
                "total_tokens": log.get('total_tokens', 0),
                "client_app": log.get('client_app', 'Unknown'),
                "cost": log.get('cost', 0)
            })
        ...
        return {
            "user": {
                ...
                "avatar_url": avatar_url,
                "sub_key_prefix": sub_key_info.get('sub_key_prefix', ''),
                "enabled": sub_key_info.get('enabled', True),
                "created_at": str(sub_key_info.get('created_at')) if sub_key_info.get('created_at') else None,
                "last_used_at": str(sub_key_info.get('last_used_at')) if sub_key_info.get('last_used_at') else None,
                "master_key_name": sub_key_info.get('master_key_name', '')
            },
            "usage": {
                "daily_dollar_usage": dollar_usage.get('current_usage', 0),
                "daily_dollar_limit": dollar_usage.get('limit', 20.0),
        ...
        logger.error(f"Error getting sub-key details: {e}")
        await broadcast_log(f"Error getting sub-key details: {str(e)}", "ERROR")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_key_models(api_key: str):
    ...
    try:
        ...
        # Get target URL, API key, no_auth flag, and http_referer
